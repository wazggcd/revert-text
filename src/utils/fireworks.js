export function fireworks() {
  ...
  function drawFireworks() {
    ...
    for(var i = 0 ;i<particles.length;i++){
      ...
      p.x += vx;p.y += vy;
      p.radius *= 1 - p.speed/100;p.alpha -= 0.005;
      context.beginPath();
  ...
  const rect = document.getElementById('revertShow').getBoundingClientRect()
  fire(canvas.width * 0.5, (rect.top + rect.bottom)/2)
  setTimeout(() => {
    fire(canvas.width * (1/3), 200)
  }, 3.5 * 1000)
}
